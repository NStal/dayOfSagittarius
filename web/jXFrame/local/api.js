...
exports.distributer = function(apiRequest){
	...
	if(typeof api.auth == "function"){
		...
		api.auth.call(apiRequest,function(data){
		    console.log("log auth call back");
...
apiRequest.prototype.removeCookie = function(key){
    //set cookie expired; 
    this._cookieTobeDelete.push(key);
    var expireArray = [];
    var expireDate = new Date(1970,1);
    for(var i=0;i<this._cookieTobeDelete.length;i++){
	expireArray.push(this._cookieTobeDelete[i]
			 +"=None; "+"expires="+expireDate);
    }
