...
DockFacility.prototype._init = function (info){
    ...
    this.guidButtonJ = dock.guidBox[this.name+"ButtonJ"];
    ...
    this.guidButtonJ.mouseover(function (){
	var previewBox = dock.guidBox.previewBox;
	previewBox.show(
	    self.guidButtonJ[0].offsetTop
		+ previewBox.nodeJ.height() / 2
		+ self.guidButtonJ.height() / 2
	);
    })
    this.guidButtonJ.click(function (){
	self.getIn();
    })
}
...
DockFacility.prototype.registRole = function (){
	...
	this.interactionBox.rolePicBoxJ.hide();
	this.interactionBox.diologBoxJ.hide();
    }
}
...
//GameServer: node root/server/server.js//wasted code from star stationscene
//just copy in case
